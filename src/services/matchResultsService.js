...
async function fetchMatchesForLobby(lobby) {
    const matches = [];
    let currentPage = 1;
    ...
    try {
        while (matches.length < lobby.numberOfGames) {
            const response = await axios.get(
                `https://supervive.op.gg/api/players/steam-d89452e250ce4158b7bf3a15ed11147e/matches?page=${currentPage}`
            );
...
async function setLobbyMatches(lobbyName, endDate = null) {
    try {
        const lobbyRepository = AppDataSource.getRepository("Lobby");
        const lobby = await lobbyRepository.findOne({
        ...
        const matches = await fetchMatchesForLobby(lobby);
