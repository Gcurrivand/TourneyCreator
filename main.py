...
async def list_players(ctx):
    ...
    for i, (username, rank, hunters, is_otp, status, position) in enumerate(players, 1):
        if status == 'queued' and not queue_started:
        ...
        if status == 'registered':
            line = f"{i}. {username} ({rank}) - Hunters: {hunters}{otp_status}\n"
        else:
            line = f"Q{position}. {username} ({rank}) - Hunters: {hunters}{otp_status}\n"
...
async def start_tournament(ctx, *, date_string=None):
    ...
    Optional: specify start date, end date, and number of games
    Examples:
    !start                           # Starts now, no end date, 3 games
    !start 2024-03-20 18:00         # Starts at date, no end date, 3 games
    !start 2024-03-20 18:00 2024-03-21 18:00  # Starts and ends at specified dates, 3 games
    !start 2024-03-20 18:00 2024-03-21 18:00 5  # Starts and ends at specified dates, 5 games
    """
    ...
    if date_string:
        try:
            ...
            if len(parts) >= 5:  # Both dates and number of games provided
                start_date_str = f"{parts[0]} {parts[1]}"
                end_date_str = f"{parts[2]} {parts[3]}"
                number_of_games = int(parts[4])
                ...
                if tournament_end <= tournament_start:
                    ...
                    return
            elif len(parts) >= 4:  # Both start and end dates provided
                start_date_str = f"{parts[0]} {parts[1]}"
                end_date_str = f"{parts[2]} {parts[3]}"
                tournament_start = datetime.strptime(start_date_str, "%Y-%m-%d %H:%M")
                tournament_end = datetime.strptime(end_date_str, "%Y-%m-%d %H:%M")
...
async def register_player(ctx, *, registration_text):
    ...
    # List of valid ranks for parsing
    valid_ranks = ['bronze', 'silver', 'gold', 'platine', 'diamant', 'master', 'gm', 'legend']
...
async def randomize_teams(ctx):
    """Creates balanced teams from registered players"""
    result = tournament_manager.create_balanced_teams()

    tournament_manager.save_teams(result)

    # Format teams into messages
    messages = []
    current_message = "**Balanced Teams:**\n\n"
